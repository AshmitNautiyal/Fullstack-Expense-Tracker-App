...
import {maskAccountNumber , formatCurrency} from '../libs/index.js';
    ...
    <div className = "w-full py-10">
      <div className = "flex items-center justify-between">
        <Title title = "Accounts Information" />
          <div className = "flex items-center gap-4">
            <button onClick = {() => setIsOpen(true)} className = "py-1.5 px-2 rounded bg-black dark:bg-violet-600 text-white dark:text-white flex items-center justify-center gap-2 border border-gray-500">
              <MdAdd size = {22} />
      ...
      </div>
      {
        data?.length == 0 ? (
          ...
          <div className = "w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 py-10">
            {data?.map((acc , index) =>(
              <div 
              key = {index}
              className = "w-full h-48 gap-4 bg-gray-50 dark:bg-slate-800 p-3 rounded shadow">
               <div>
